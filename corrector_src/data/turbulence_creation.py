...
def randomized_turbulent_initial_state(
    num_cells: int, cfg_data: DictConfig, mhd: bool, rng_seed: Optional[int] = None
) -> Tuple[
    np.ndarray, SimulationConfig, SimulationParams, HelperData, RegisteredVariables, int
]:
    ...
    u_x = create_turb_field(
        config.num_cells, 1, turbulence_slope, kmin, kmax, seed=seeds[0]
    )
    u_y = create_turb_field(
        config.num_cells, 1, turbulence_slope, kmin, kmax, seed=seeds[1]
    )
    u_z = create_turb_field(
        config.num_cells, 1, turbulence_slope, kmin, kmax, seed=seeds[2]
    )
