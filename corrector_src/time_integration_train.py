...
def time_integration(
    ...
    if config.runtime_debugging:
        ...
        checked_integration = checkify.checkify(_time_integration_train, errors)
        ...
        return _time_integration_train(primitive_state, config, params, helper_data, registered_variables, snapshot_callable, train)
...
(time, state, snapshot_data)
 ...
 ┌──────┼───────────────────────────┐
 │      ▼                           │
 │  maybe update snapshot_data      │
 │      ▼                           │
 │  compute dt ------------------┐  │
 │      ▼                        │  │
 │  evolve state (physics)       │  │
 │      ▼                        │  │
 │  increment time               │  │
 │      ▼                        │  │
 │  maybe record final snapshot  │  │
 │      ▼                        │  │
 └─────► return (time, state, snapshot_data)

"""
@jaxtyped(typechecker=typechecker)
@partial(jax.jit, static_argnames=['config', 'registered_variables', 'snapshot_callable'])
def _time_integration_train(
    primitive_state: STATE_TYPE,
    config: SimulationConfig,
    params: SimulationParams,
    helper_data: HelperData,
