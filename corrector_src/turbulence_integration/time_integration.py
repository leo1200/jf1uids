...
from corrector_src.turbulence_integration.simulation_snapshot_data import SnapshotData
from corrector_src.turbulence_integration.turbulent_energy import (
    calculate_turbulent_energy,
    _evolve_turb_energy,
)
...
def _time_integration(
    ...
    if config.return_snapshots:
        ...
        turbulent_energy = jnp.zeros(config.num_snapshots)
        radial_momentum = (
            jnp.zeros(config.num_snapshots)
        ...
        snapshot_data = SnapshotData(
            ...
            turbulent_energy=turbulent_energy,
            final_state=None,
        )
    ...
    def update_step(carry):
        if config.return_snapshots:
            ...
            def update_snapshot_data(time, state, snapshot_data):
                ...
                turbulent_energy = snapshot_data.turbulent_energy.at[
                    snapshot_data.current_checkpoint
                ].set(
                    calculate_turbulent_energy(
                        unpad_state, registered_variables, config
                    )
                )
        ...
        if config.sgs_turb_energy:
            state = _evolve_turb_energy(
                state,
                dt,
                params.gamma,
                config,
