...
masses = jnp.array([0.5, 0.5], dtype=jnp.float64)  # Masses of the two point masses
...
M1 = 0.5                  # primary mass (M⊙)
M2 = 0.5                  # secondary mass (M⊙)
Mtot = M1 + M2

R_cav = 2.5               # cavity radius [au]
delta_0    = 3e-4      #1e-5        # floor parameter
sigma_0    = 5e-3      # choose normalization for Σ(r)
rsafe = 1e-6
...
#     1e-4)
rho = jnp.clip(sigma / (H * jnp.sqrt(2 * jnp.pi)) * jnp.exp(-0.5 * (z_cyl / H)**2),a_min=1e-6)
...
# Gas pressure 
P = jnp.clip(rho * cs**2,a_min=1e-8)
...
v_phi = jnp.sqrt(
    jnp.clip(G * Mtot / (r_cyl + rsafe) + (r_cyl / (rho + rho_safe)) * dP_dr, a_min=0.0)  # avoid negative values
)
...
plt.xlabel("x")
plt.ylabel("y")
plt.savefig("Binary_paper/initial_rho.png")
plt.close()
...
plt.xlabel("x")
plt.ylabel("z")
plt.savefig("Binary_paper/initial_rho_side.png")
plt.close()
...
plt.tight_layout()
fig.savefig("Binary_paper/initial_variables.png")
plt.close()
...
plt.xlabel("x")
plt.ylabel("y")
plt.savefig("Binary_paper/endstate.png")
...
plt.tight_layout()
fig.savefig("Binary_paper/var_finals.png")
...
# save to gif
ani.save("Binary_paper/rho_ani.gif")
plt.close(fig)
...
plt.colorbar(ax.imshow(result.states[0][0, num_cells // 2, :, :].T, cmap="jet", origin="lower", extent=[0, box_size, 0, box_size], norm=LogNorm()), ax=ax)
# save to gif
ani.save("Binary_paper/rho_ani_side.gif")
plt.close(fig)
