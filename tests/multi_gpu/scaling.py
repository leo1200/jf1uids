...
def measure_execution_time(primitive_state, config, params, helper_data, registered_variables, sharding = None):
    # execute once for compilation and warmup

    final_state = time_integration(primitive_state, config, params, helper_data, registered_variables, sharding = sharding)
...
def make_scaling_plots(sharding = False, num_cells_list = [32, 64, 128, 256, 512, 1024]):
    ...
    for num_cells in num_cells_list:
        ...
        execution_time = measure_execution_time(primitive_state, config, params, helper_data, registered_variables, sharding = named_sharding)
