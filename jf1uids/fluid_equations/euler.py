...
def _euler_flux(primitive_states: STATE_TYPE, gamma: Union[float, Float[Array, ""]], config: SimulationConfig, registered_variables: RegisteredVariables, flux_direction_index: int) -> STATE_TYPE:
    ...
    utotal = get_absolute_velocity(primitive_states, config, registered_variables)
    ...
    if config.dimensionality == 1:
        flux_vector = flux_vector.at[registered_variables.velocity_index].set(primitive_states[registered_variables.velocity_index] * rho)
