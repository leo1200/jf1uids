...
def boundary_handler(primitive_states, left_boundary, right_boundary):
    if left_boundary == OPEN_BOUNDARY:
        primitive_states = open_left_boundary(primitive_states)
    elif left_boundary == REFLECTIVE_BOUNDARY:
        primitive_states = reflective_left_boundary(primitive_states)
    else:
        raise ValueError("Unknown left boundary condition")
    
    if right_boundary == OPEN_BOUNDARY:
        primitive_states = open_right_boundary(primitive_states)
    elif right_boundary == REFLECTIVE_BOUNDARY:
        primitive_states = reflective_right_boundary(primitive_states)
    else:
...
@jax.jit
def open_right_boundary(primitive_states):
...
@jax.jit
def open_left_boundary(primitive_states):
...
@jax.jit
def reflective_left_boundary(primitive_states):
...
@jax.jit
def reflective_right_boundary(primitive_states):
