...
def get_helper_data(config: SimulationConfig, sharding: Union[NoneType, NamedSharding] = None, padded: bool = False) -> HelperData:
    ...
    if config.geometry == SPHERICAL or config.geometry == CYLINDRICAL:
        ...
        if config.dimensionality > 1:
            x = jnp.linspace(grid_spacing / 2 - ngc * grid_spacing, config.box_size - grid_spacing / 2 + ngc * grid_spacing, config.num_cells + 2 * ngc)
            y = jnp.linspace(grid_spacing / 2 - ngc * grid_spacing, config.box_size - grid_spacing / 2 + ngc * grid_spacing, config.num_cells + 2 * ngc)
