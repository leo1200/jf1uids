...
from jf1uids.option_classes.simulation_config import AM_HLLC, HLL, HLLC, HLLC_LM, HYBRID_HLLC, LAX_FRIEDRICHS, STATE_TYPE, STATE_TYPE_ALTERED, SimulationConfig

@jaxtyped(typechecker=typechecker)
@partial(jax.jit, static_argnames=['config', 'registered_variables', 'flux_direction_index'])
def _riemann_solver(primitives_left: STATE_TYPE, primitives_right: STATE_TYPE, primitive_state: STATE_TYPE_ALTERED, gamma: Union[float, Float[Array, ""]], config: SimulationConfig, registered_variables: RegisteredVariables, flux_direction_index: int) -> STATE_TYPE:
